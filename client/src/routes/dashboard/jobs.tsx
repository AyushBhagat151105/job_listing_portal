    ...
    if (authPending || isProfileLoading || isJobsLoading) {
        return (
            <div className="max-w-5xl mx-auto px-4 py-8 space-y-4">
                <Skeleton className="h-10 w-64 bg-zinc-800 mb-8" />
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            {!profile ? (
                ...
                <Card className="border-zinc-800/60 bg-zinc-900/60 text-center py-12">
                    <CardContent className="space-y-4">
                        ...
                        <h3 className="text-lg font-semibold text-zinc-200">No Job Listings Yet</h3>
                        <p className="text-zinc-400 max-w-md mx-auto">
                ...
                <div className="grid gap-4">
                    {jobs.map((job) => (
                        <Card key={job.id} className="border-zinc-800/60 bg-zinc-900/60 hover:bg-zinc-800/40 transition-colors">
                            <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    ...
                                    <div className="flex items-start justify-between">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 mt-1.5 text-sm text-zinc-500">
                                                ...
                                                <span className="flex items-center gap-1.5">
                                                    <IndianRupee size={14} />
                                                    {job.salaryMin ? `${job.salaryMin.toLocaleString()} - ${job.salaryMax?.toLocaleString()}` : 'Not specified'}
                                                </span>
                                            </div>
                                        </div>
                                        <Badge variant="outline" className={
                                            job.status === 'ACTIVE' ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10' :
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-3 md:pl-6 md:border-l border-zinc-800">
